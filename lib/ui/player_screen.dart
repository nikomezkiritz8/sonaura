...
import '../models/lyric_model.dart'; // Asegúrate de tener este modelo
import '../services/qobuz_service.dart';
import '../services/sonaura_ai.dart';
import '../services/voice_service.dart';
import '../services/lyrics_service.dart'; // Asegúrate de tener este servicio
...
class _PlayerScreenState extends State<PlayerScreen> with TickerProviderStateMixin {
  ...
  void _prepararAudio() async {
    ...
    setState(() {
      _isBuffering = true;
      _lyrics = null; // Reseteamos letras al cambiar de canción
    });

    final track = widget.playlist[_currentIndex];

    // BUSCAR LETRAS EN SEGUNDO PLANO
    _lyricsService.getLyrics(track.artist, track.cleanTitle).then((l) {
  ...
  void _obtenerInsight() async {
    ...
    showModalBottomSheet(
      context: context, backgroundColor: Colors.black.withOpacity(0.9),
  ...
  @override
  void dispose() { 
    _audioPlayer.dispose(); _visualizerController.dispose(); _bgController.dispose(); 
    _lyricsScrollController.dispose(); super.dispose(); 
  }
      ...
      body: Stack(
        children: [
          ...
          SafeArea(
            child: Column(
              children: [
                // 2. APPBAR ACTUALIZADO CON BOTÓN DE LETRAS
                AppBar(
                  ...
                  actions: [
                    // BOTÓN DE LETRAS
                    IconButton(
                      icon: Icon(Icons.lyrics_outlined, color: _showLyrics ? SonauraColors.accentGold : Colors.white38),
                      onPressed: () => setState(() => _showLyrics = !_showLyrics),
                    ),
                    IconButton(icon: const Icon(Icons.psychology, color: SonauraColors.accentGold, size: 28), onPressed: _obtenerInsight),
                ...
                // 3. CUERPO CENTRAL (CARÁTULA O LETRAS)
                Expanded(
                  flex: 12,
                  child: AnimatedSwitcher(
                    duration: const Duration(milliseconds: 500),
                    child: _showLyrics ? _buildLyricsView() : _buildCoverView(track),
                  ),
                ),
                ...
                // 4. ONDA SPECTRAL
                Container(
                  height: 40, width: double.infinity, padding: const EdgeInsets.symmetric(horizontal: 70),
                  child: AnimatedBuilder(
                    animation: _visualizerController, 
                    builder: (c, _) => CustomPaint(
                      painter: SpectrumPainter(animationValue: _visualizerController.value, isPlaying: _audioPlayer.playing)
                    )
                  ),
                ),
  ...
  // --- VISTA DE CARÁTULA ---
  Widget _buildCoverView(SonauraTrack track) {
    return Center(
      child: Hero(
        tag: track.id,
        child: Container(
  ...
  Widget _buildLyricsView() {
    if (_lyrics == null) return const Center(child: Text("Buscando letras sincronizadas...", style: TextStyle(color: Colors.white24, fontStyle: FontStyle.italic)));
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final isCurrent = currentPos >= line.time && (index == _lyrics!.lines.length - 1 || currentPos < _lyrics!.lines[index + 1].time);
            ...
            return AnimatedDefaultTextStyle(
              ...
              style: TextStyle(
                color: isCurrent ? SonauraColors.accentGold : Colors.white,
                fontSize: isCurrent ? 26 : 20,
                fontWeight: isCurrent ? FontWeight.bold : FontWeight.w200,
                height: 1.8,
              ),
              child: Padding(
...
class SpectrumPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    var paint = Paint()..color = SonauraColors.accentGold.withOpacity(0.5)..style = PaintingStyle.fill;
